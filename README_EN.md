...
- **Automatic disambiguation of shared models**: when the same model ID is offered by several enabled providers (e.g. both International and China editions are signed in), `/v1/models` lists one `provider:model` entry per provider so you can pick the exact copy; models unique to a single provider keep their plain ID.
